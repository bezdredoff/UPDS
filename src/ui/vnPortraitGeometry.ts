 ...
 * for duo/trio shots and then refines the same formula against the measured focal
...
export function resolveVnPortraitEyeLineCamera(
  ...
  if (!Number.isFinite(eyeLineYPx) || eyeLineYPx <= 0 || eyeLineYPx >= VN_MASTER_CANVAS_HEIGHT_PX) {
    throw new Error(`VN portrait eye-line must be inside the ${VN_MASTER_CANVAS_HEIGHT_PX}px master canvas, got ${eyeLineYPx}`);
  }
  ...
  const heightPercent = rounded(VN_RUNTIME_PORTRAIT_HEIGHT_PERCENT * shotScale);
  const eyeLineRatio = eyeLineYPx / VN_MASTER_CANVAS_HEIGHT_PX;
