...
Статус: active production test-policy contract, reconciled at ANM-028B1 R4.
...
- Scene Studio regression tests must prove that scene actors use the playable `.portrait` crop,
  face-critical safe lanes and dialogue occlusion. Trio tests also verify focal-eye-line camera
  math, declared neutral eye landmarks and headroom. A separate full-body stage renderer, fixed-top
  trio camera or actor shot scale below the contracted floor must fail before mobile QA.
