...
Status: audited active architecture at the ANM-028B1 R2 repository candidate.
...
Owns the read-only ANM-028B1 R2 composition/calibration QA surface: preset/background/authored-line,
text-scale and ANM-024 viewport switching; the same shared VN frame as playable runtime; safe-area,
contain/focal/horizon/footline/actor-zone and preset guides; canonical neutral lineup; separated
...
bounds. Scene-specific CSS zoom must not become a parallel proportion system. ANM-028B1 R2 Scene
Studio previews normalized runtime coordinates inside real VN occlusion/chrome with separate nested
shot/canonical scale layers; its neutral lineup exposes bottom-pivot/alpha measurement drift without
