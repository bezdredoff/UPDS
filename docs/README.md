# UPDS documentation index

This directory is the documentation source of truth for the current repository.

## Read first

1. [`architecture/PROJECT_CONTRACTS_RU.md`](architecture/PROJECT_CONTRACTS_RU.md) — immutable product/runtime contracts.
2. [`architecture/ARCHITECTURE_RU.md`](architecture/ARCHITECTURE_RU.md) — current code ownership and data flow.
3. [`process/AI_DEVELOPMENT_RU.md`](process/AI_DEVELOPMENT_RU.md) — safe workflow for AI and developers.
4. [`process/TESTING_RU.md`](process/TESTING_RU.md) — current test strategy and acceptance gates.
5. [`ROADMAP_RU.md`](ROADMAP_RU.md) — next planned work.

## Active feature documentation

- [`features/ANM011_INFRASTRUCTURE_RU.md`](features/ANM011_INFRASTRUCTURE_RU.md)
- [`features/ANM012_MOBILE_UX_RU.md`](features/ANM012_MOBILE_UX_RU.md)
- [`features/ANM013_VN_PRE_RELEASE_UX_RU.md`](features/ANM013_VN_PRE_RELEASE_UX_RU.md)
- [`features/ANM014_MATCH3_PRE_RELEASE_UX_RU.md`](features/ANM014_MATCH3_PRE_RELEASE_UX_RU.md)
- [`features/ANM015_AUDIO_HAPTICS_RU.md`](features/ANM015_AUDIO_HAPTICS_RU.md)
- [`features/ANM016_PRESENTATION_POLISH_RU.md`](features/ANM016_PRESENTATION_POLISH_RU.md)
- [`features/ANM017_PLAYTEST_DISTRIBUTION_RU.md`](features/ANM017_PLAYTEST_DISTRIBUTION_RU.md)
- [`features/ANM018_REPOSITORY_MAINTENANCE_RU.md`](features/ANM018_REPOSITORY_MAINTENANCE_RU.md)
- [`features/ANM018A_UI_ORCHESTRATOR_DECOMPOSITION_RU.md`](features/ANM018A_UI_ORCHESTRATOR_DECOMPOSITION_RU.md)
- [`features/ANM019A_LOCALIZATION_CORE_RU.md`](features/ANM019A_LOCALIZATION_CORE_RU.md)
- [`features/ANM019B_LOCALIZED_SYSTEM_UI_RU.md`](features/ANM019B_LOCALIZED_SYSTEM_UI_RU.md)
- [`features/ANM019C_LOCALIZED_VN_METADATA_CHOICES_RU.md`](features/ANM019C_LOCALIZED_VN_METADATA_CHOICES_RU.md)
- [`features/ANM019D_LOCALIZED_SCREENPLAY_RUNTIME_RU.md`](features/ANM019D_LOCALIZED_SCREENPLAY_RUNTIME_RU.md)
- [`features/ANM019E1_LOCALIZED_DOSSIER_ENDING_RU.md`](features/ANM019E1_LOCALIZED_DOSSIER_ENDING_RU.md)

## Process

- [`process/GITHUB_PHONE_PIPELINE_RU.md`](process/GITHUB_PHONE_PIPELINE_RU.md)
- [`process/AI_DEVELOPMENT_RU.md`](process/AI_DEVELOPMENT_RU.md)
- [`process/TESTING_RU.md`](process/TESTING_RU.md)

## Current validation

- [`reports/ANM018A_VALIDATION_REPORT_RU.md`](reports/ANM018A_VALIDATION_REPORT_RU.md)
- [`reports/ANM018A_MANUAL_QA_RU.md`](reports/ANM018A_MANUAL_QA_RU.md)

## Archive

`archive/` contains historical implementation notes, old feature README files, validation reports and manual QA snapshots. They are retained for traceability only. If an archived document conflicts with `architecture/`, `process/` or `features/`, the active document wins.

- [`features/ANM019F_COMPLETE_SCREENPLAY_LOCALIZATION_RU.md`](features/ANM019F_COMPLETE_SCREENPLAY_LOCALIZATION_RU.md)
