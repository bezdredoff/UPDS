...
Status: **active release-planning source**, ANM-030B0H + ANM-030B1B1 + ANM-030B1B2 + ANM-030B1B3 + ANM-030B1B4.
...
После ANM-030B1B4 audit фиксирует `9/24` dedicated production background variants и `15` runtime aliases. Утверждённая аудитория студсовета больше не использует фон маленькой клубной комнаты, smart-textile lab больше не подменяется квартирой Норихиро, склад бюро находок — спортивной раздевалкой, а служебный двор кампуса — клубной комнатой. `lab-asterion` уже получил production master; `transfer-point` и `server-room` остаются controlled sibling variants. `laundry-service` уже получил production master; `maintenance-room` и `anonymous-return-counter` остаются controlled sibling variants. `campus-exterior` уже получил production master; `campus-path` остаётся controlled sibling variant, а не копией anchor. Оставшиеся aliases по-прежнему не являются требованием произвести столько же независимых картинок.

Обязательный outcome:

- создать одну всё ещё отсутствующую master family, которая сейчас подменена заведомо другой локацией: `old-building-finale`;
...
После одного оставшегося недостающего master добавить только те variants существующих families, которые visual QA оценивает как заметный narrative mismatch. Не закрывать оставшийся счётчик `15 aliases` ради самого счётчика.
...
2. **Release-critical visual production:** один still-missing background master + controlled Asterion/laundry/campus sibling variants при подтверждённом mismatch + production presentation для шести guests; интегрировать небольшими reviewable waves с iPhone preview.
