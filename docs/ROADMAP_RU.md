...
Active production foundation: **ANM-025/026 Match-3 production + tooling, completed ANM-027A–G canonical story pipeline, accepted ANM-028B1 R4.1 Scene Studio geometry, ANM-028B2 R1.1 authored VN shot adoption, ANM-028B3 R1.1 guest/witness presentation and ANM-028D3A Emi approved-frame runtime transition**. Current candidate focus: **ANM-029B1 R1 Belarusian Player Shell**. Remaining character-art generation is paused for an external Stable Diffusion workflow; the complete `0–21` screenplay is now a frozen input for localization production.
...
- **029B Belarusian Production — IN PROGRESS** — B1 R1 player-shell translation is in QA (61 bounded `localization/common/menu/settings/audio/pwa` keys); B2 Match-3 production is next, followed by VN/ending batches and runtime enablement only after the full zero-missing-key audit;
