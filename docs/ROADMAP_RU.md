...
- **ANM-028B1 R4 Focal Eye-Line Staging & Asset QA — IN QA** — preserves the eight
  `upds-scene-staging-v1` IDs and shared VN frame, keeps solo/two-shot runtime framing, and replaces
  the rejected trio top-lock with measured character eye landmarks aligned to each rendered
  background focal point. Lineup now separates runtime integration from manual visual approval.
...
- **027F Full Story Macro Lock — NEXT AFTER 028B1 R4 AND EMI NEUTRAL BASELINE** — reconcile `ANM-001 Story Bible v0.2`, `ANM-002 22-Episode Plot v0.1` and the historical 115-slide `UPDS.pptx` into a `0–21` beat/location/cast/clue/Match-3/transition/asset-trigger map before detailed writing; use ANM-027E budgets instead of ANM-002 §8 production estimates and only the preset IDs frozen by accepted 028B1 R4;
...
- **028B Character/Scene Studio 2.0 — IN PROGRESS; 028B1 R4 IN QA** — reusable preset registry exists, but completion waits for focal-eye-line trio, viewport, lineup and background-calibration visual acceptance; background/shot/actor-position/expression/Pose B authoring remains 028B2 and guest presentation remains 028B3;
...
- **028B1 R4 Focal Eye-Line Staging & Asset QA — IN QA** — canonical `upds-scene-staging-v1` registry/resolver plus shared playable VN chrome, eye-line-anchored trio camera, ANM-024 viewport matrix, contain-over-fill/background guides, visual-status lineup, diagnostics and read-only QA report; no new art; authored VN adoption remains 028B2;
...
3. **ANM-028B1 R4 Focal Eye-Line Staging & Asset QA — IN QA**; accept it through candidate CI + iPhone visual QA before freezing preset geometry;
