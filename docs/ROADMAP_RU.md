...
Active production foundation: **ANM-025 Match-3 production framework + ANM-026 tooling + ANM-027 story architecture/import pipeline for currently authored canon + ANM-027E lean content production contract + ANM-028B1 reusable staging presets/Scene Studio**. Current production focus after 028B1 acceptance: **ANM-027F full-story macro lock**. Full screenplay production beyond the ANM-003 vertical slice remains pending before ANM-029/release.
...
- ANM-028B1 Reusable Staging Presets & Scene Budget Preview: canonical `upds-scene-staging-v1` registry with eight solo/two-shot/trio/evidence/guest compositions, normalized resolver, safe-area/non-overlap validator and read-only Scene Studio using existing art/backgrounds only.
- documentation/traceability authority aligned with ANM-028B1 and protected by a focused docs drift gate.
...
- **027F Full Story Macro Lock — NEXT** — reconcile `ANM-001 Story Bible v0.2`, `ANM-002 22-Episode Plot v0.1` and the historical 115-slide `UPDS.pptx` into a `0–21` beat/location/cast/clue/Match-3/transition/asset-trigger map before detailed writing; use ANM-027E budgets instead of ANM-002 §8 production estimates and the approved ANM-028B1 preset IDs for staging;
...
- **028B Character/Scene Studio 2.0 — IN PROGRESS; 028B1 COMPLETE** — reusable preset preview is stable; background/shot/actor-position/expression/Pose B authoring plus shared-baseline lineup/proportion ruler remain in 028B2/028B3 using the same runtime coordinates;
...
- **028B1 Reusable Staging Presets & Scene Budget Preview — COMPLETE** — canonical `upds-scene-staging-v1` solo/two-shot/trio/evidence/guest-testimony registry, runtime-coordinate resolver, safe-area/non-overlap validation and Scene Studio preview; no new mass art; authored VN adoption is intentionally deferred to 028B2;
...
3. **ANM-028B1 Reusable Staging Presets & Scene Budget Preview — COMPLETE**;
4. **ANM-027F Full Story Macro Lock — NEXT** from the approved Story Bible and original scenario presentation, preserving all slots `0–21` and three endings and assigning only approved staging preset IDs;
