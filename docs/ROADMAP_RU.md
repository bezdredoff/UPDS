...
- **ANM-030B0A2 / B0B / B1–B4 graphics and art integration** is no longer blocked by browser automation; it remains product-gated and starts only when external/approved art production is explicitly resumed;
...
### ANM-023G — Playwright Browser Automation [P0/P1] — COMPLETE THROUGH G7B / PR #158

Playwright is the browser/E2E framework. The finished track reuses production QA/product surfaces rather than creating a second game implementation, and keeps browser CI parallel to the existing `npm run check` quality gate.
...
**023G7C Version / Diagnostics Closeout [P1] — CURRENT CANDIDATE** closes stale product/build/save-schema presentation and roadmap status after G7B; it does not add browser coverage or gameplay behavior.
...
- **ANM-030B0A2 [P1] — PLANNED / READY WHEN ART PRODUCTION RESUMES: Match-3 Special/Bonus Asset Integration.** Import five externally approved production PNGs, switch `specialAssets` from SVG fallback to production assets, preserve preload/offline safety and run playable mobile board QA. No per-level bonus packs.
- **ANM-030B0B [P1] — PLANNED / READY WHEN ART PRODUCTION RESUMES: Character Closure.** Emi `embarrassed` / Pose B / medallion replacements plus seven extras semantic roles mapped onto ≤4 reusable visual archetypes.
- **ANM-030B1–B4 [P1] — PLANNED / PRODUCT-GATED:** vertical story-route production waves for remaining characters, guests, background masters/variants and six hero clues.
...
16. **ANM-023G7C R1 [P1] — CURRENT CANDIDATE** — product-version, diagnostics/save-schema display and roadmap identity closeout;
17. **ANM-030B0A2 [P1] — PLANNED / READY WHEN ART PRODUCTION RESUMES** — integrate five externally approved production special/bonus PNGs and perform mobile board QA;
18. **ANM-030B0B [P1] — PLANNED / PRODUCT-GATED** — Emi/extras character closure;
19. **ANM-030B1–B4 [P1] — PLANNED / PRODUCT-GATED** — budgeted story-route art/content integration using the external character/background workflow;
20. **ANM-031 [P2]** — landscape;
21. **ANM-032 [P2]** — music;
22. **ANM-033 [P0 before release]** — release hardening.
