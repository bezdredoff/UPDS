...
Геометрия preset считается замороженной только после ANM-028B1 R4 QA внутри общего playable VN
frame: с реальными header/dialogue/controls, ANM-024 viewport matrix, playable `.portrait` crop,
trio focal-eye-line alignment и runtime `contain-over-fill`. Actor safe lanes защищают лица, а не полный прозрачный PNG; нижняя
...
2. **ANM-028B1 R4 — IN QA** — reusable `upds-scene-staging-v1`, shared playable VN frame,
   focal-eye-line trio crop, viewport/background calibration, visual-status lineup и read-only QA
   brief без нового art. R3/PR #94 proved that fixed-top trio scaling and Emi baseline are invalid.
3. **ANM-028D0 — NEXT AFTER R4 ACCEPTANCE** — один новый Emi neutral full-body master; expressions,
