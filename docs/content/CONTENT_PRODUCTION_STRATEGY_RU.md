...
Геометрия preset считается замороженной только после ANM-028B1 R2 QA внутри общего playable VN
frame: с реальными header/dialogue/controls, ANM-024 viewport matrix и runtime
`contain-over-fill`. Neutral lineup и background horizon/footline/actor-zone calibration являются
обязательными acceptance inputs. Runtime scale или episode-specific CSS нельзя использовать для
маскировки неправильного character master/background perspective.
...
2. **ANM-028B1 R2 — IN QA** — reusable `upds-scene-staging-v1`, shared playable VN frame,
   viewport/background calibration, neutral lineup и read-only QA brief без нового mass art.
3. **ANM-027F — NEXT AFTER R2 ACCEPTANCE** — macro lock `0–21` и asset-trigger map на основе Story
