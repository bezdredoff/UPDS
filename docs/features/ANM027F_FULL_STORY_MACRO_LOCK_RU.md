...
Status: **COMPLETE**. The original R1 macro contract is being consumed incrementally by ANM-027G; all slots `0–21` are now authored/production-configured after the final ANM-027G ending batch.
...
2. contiguous authored prefix versus remaining macro-locked slots (after ANM-027G `19–21`: authored `0–21`, no remaining macro-only slots);
...
- packages `4–6`, `7–9`, `10–12` and `13–15` are implemented detailed screenplay batches; `16–18` is next.
