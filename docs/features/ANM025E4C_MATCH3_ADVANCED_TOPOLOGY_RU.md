...
Build label: `ANM-025E4C R1 · Advanced Topology Cohort`.
...
`boardHoles = [9, 10, 11, 12, 20, 21, 22, 23, 60, 61, 62, 63]` — 52 active cells.

Три смещённых горизонтальных пропуска читаются как проходы между архивными полками, не копируя split/workbench формы E4B. Каталог Рины остаётся на активной центральной позиции.
...
`boardHoles = [0, 7, 17, 23, 24, 30, 33, 39, 40, 46, 56, 62]` — 52 active cells.
...
Это не новый balance target для игрока и не замена human playtest. Это regression envelope: topology не должна незаметно сделать поздние levels хуже для уже установленного deterministic comparator.
