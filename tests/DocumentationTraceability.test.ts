...
describe('active documentation traceability', () => {
  ...
  it('documents the canonical story import and current missing-content boundary', () => {
    ...
    expect(protectedContracts).toContain('619 authored lines');
    expect(protectedContracts).toContain('VN0250` является canonical bridge');
    expect(index).toContain('slots `0–12`');
    expect(index).toContain('Slots `13–21` remain macro-only');
  });
  ...
  it('locks the lean full-content scope and keeps guest/offline production boundaries explicit', () => {
    ...
    expect(aiWorkflow).toContain('`4–6`, `7–9` and `10–12` are implemented packages');
    expect(aiWorkflow).toContain('`13–15` is the next package');
