...
function readWebpDimensions(buffer: Buffer): readonly [number, number] {
  ...
  while (offset + 8 <= buffer.length) {
    ...
    if (fourcc === 'VP8X') {
      return [1 + buffer.readUIntLE(dataOffset + 4, 3), 1 + buffer.readUIntLE(dataOffset + 7, 3)];
    }
...
describe('ANM-030B1B7 basketball locker production background', () => {
  it('uses the dedicated production asset instead of the athletics fallback', () => {
  ...
  it('keeps both E5 VN scenes on the basketball semantic key', () => {
  ...
  it('ships the exact approved 1080x1920 WebP imported by the binary-safe ZIP', () => {
