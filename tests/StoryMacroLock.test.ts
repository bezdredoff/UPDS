...
describe('ANM-027F full-story macro lock', () => {
  it('locks all 22 slots, the authored boundary and the three endings', () => {
    ...
    expect(macro.slots.filter((slot) => slot.contentStatus === 'authored').map((slot) => slot.slot)).toEqual([0, 1, 2, 3]);
    expect(macro.slots.filter((slot) => slot.contentStatus === 'macro-locked').map((slot) => slot.slot)).toEqual(Array.from({ length: 18 }, (_, index) => index + 4));
  ...
  it('locks the six downstream screenplay batches without pretending future content already exists', () => {
    ...
    expect(macro.slots.slice(0, 4).every((slot) => slot.match3.status === 'production-configured')).toBe(true);
    expect(macro.slots.slice(4).every((slot) => slot.match3.status === 'planned-config')).toBe(true);
