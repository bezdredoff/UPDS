...
describe('level definitions', () => {
  it('contains seven valid and distinct production levels', () => {
  ...
  it('preserves the established move budgets and keeps new macro-locked levels within the three-objective cap', () => {
    expect(levels.map((level) => level.moves)).toEqual([24, 26, 25, 27, 28, 27, 29]);
    ...
    expect(levels.slice(4).map((level) => level.objectives.length)).toEqual([3, 3, 3]);
